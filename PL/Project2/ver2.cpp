...
void PrintSExp( string token ) {
  if ( IsSpecialtoken( token ) ) { // 是有意義的token 
    cout << "#<procedure " << token << ">" ;
  } // if
...
bool ReBuildTree( Nodepoint &head ) {
  if ( head != NULL && head -> left != NULL ) {
    ...
    if ( head -> left -> token == "define" ) {
      ...
      ReBuildTree( temp ) ; // 把要define 的樹給建好 
      ...
      while ( walk -> left -> token == "nil" || walk -> left -> token == "#f" ) {
        temp = temp -> right ;
        walk = temp -> left ;
        if ( temp -> right -> token == "nil" && walk -> left -> token == "else" ) // 走到底 然後是 if else 的 if 
          walk -> left -> token = "#t" ;
