...
void Check_Symbol( Nodepoint &head ) { // 指展開一格  
  if ( head != NULL ) {
...
void Check_AllSymbol( Nodepoint &head ) {
  if ( head != NULL ) {
    Check_AllSymbol( head -> left ) ; 
    Check_AllSymbol( head -> right ) ;
    Check_Symbol( head ) ;
  } // if
} // Check_AllSymbol()
  ...
  if ( head != NULL && head -> left != NULL ) {
    if ( ReBuildTree( head -> left ) ) {
      ...
      if ( head -> left -> token == "define" ) {
        ...
        Check_Symbol( head -> right -> left ) ;
        if ( ReBuildTree( head -> right -> left ) ) {
          if ( head -> right -> right -> token == "nil" ) { // 表示只有一個參數 
